...
static void CreateNode(const FieldDescriptor* field,
    ...
    case FieldDescriptor::CPPTYPE_INT64:
        (*node)[field->name()] = Int64ToString(reflection->GetInt64(message, field));
        break;
    case FieldDescriptor::CPPTYPE_UINT32:
        (*node)[field->name()] = reflection->GetUInt32(message, field);
        break;
    case FieldDescriptor::CPPTYPE_UINT64:
        (*node)[field->name()] = Uint64ToString(reflection->GetUInt64(message, field));
        break;
...
bool ProtoJsonFormat::ParseFromString(const std::string& input, Message* pb) {
    ...
    if (!reader.parse(input, root, false)) {
      LOG(ERROR) << "not a json object:" << input;
      return false;
    }
