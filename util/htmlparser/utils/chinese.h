...
#define IS_GB18030_4BYTES(s)	\
	( IN_RANGE((s)[0], 0x81, 0xFE) && IN_RANGE((s)[1], 0x30, 0x39) && \
	  IN_RANGE((s)[2], 0x81, 0xFE) && IN_RANGE((s)[3], 0x30, 0x39) )

/**
 * @brief 返回字的长度. 若为GBK编码，则为两个字节； GB18030四字节编码，为4个字节; ASCII码，为一个字节.
**/
#define GET_CHR_LEN(p)	(IS_GBK(p)?2:(IS_GB18030_4BYTES(p)?4:1))
