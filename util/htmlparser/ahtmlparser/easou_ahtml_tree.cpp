...
area_tree_t * area_tree_create(area_config_t *cfg)
{
	...
	if(!nodepool_init(&(atree->np), sizeof(html_area_t), DEFAULT_AREA_BLOCK_SIZE)){
		goto FAIL;
	}
...
static unsigned int check_pos(html_vnode_t *vnode, dividing_t *divider)
{
	...
	if((ydiff >= divider->hx||divider->last_valid&&divider->last_valid->hpNode->html_tag.tag_type==TAG_BR )&& ydiff > 0){
		SET_TO_LOWER(pos);
	}
...
static observer_t check_fit_prev(html_vnode_t *vnode,dividing_t *divider)
{
	...
	if (vnode->hpNode->html_tag.tag_type == TAG_BR)
	{
		...
		{
			//当前节点是br，WAP不继续融合
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> FIT_LOWER & AREA_NEW_BEGIN", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
		...
		else
		{
			...
			if(divider->last_valid&&divider->last_valid->ypos<=vnode->ypos){
				debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> FIT_LOWER & AREA_CONTINUE", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
				obsv.fit_pos = FIT_LOWER;
	...
	if(divider->begin == NULL){
		obsv.fit_pos = NO_FIT;
		debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> NO_FIT", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
		return obsv;
	}
	...
	if(IS_TO_LOWER(pos)&&divider->last_valid){
		...
		if(divider->last_valid&&(IS_TEXT_VNODE(divider->last_valid->property)||divider->last_valid->hpNode->html_tag.tag_type==TAG_BR)&& IS_TEXT_VNODE(vnode->property)){
			obsv.fit_pos = FIT_LOWER;
			//obsv.act = AREA_CONTINUE;
				debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> act AREA_CONTINUE", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
				return obsv;
			}

		if(abs(wxdiff)*100 <= divider->wx*30 || 0 == divider->wx || 0 == vnode->wx){
			obsv.fit_pos = FIT_LOWER;
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> FIT_LOWER", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			return obsv;
		}
	}
	//同样的策略应对左、右两种情况
	if(IS_TO_RIGHT(pos)){
		...
		if(vnode->hx < 40 && divider->hx < 40)
		{
			...
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> FIT_RIGHT", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			return obsv;
		}else if(abs(hxdiff)*100 <= divider->hx*30 || 0 == divider->hx || 0 == vnode->hx){
			obsv.fit_pos = FIT_RIGHT;
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> FIT_RIGHT", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			return obsv;
		}
		...
		int hxdiff = divider->hx - vnode->hx;
		if(abs(hxdiff)*100 <= divider->hx*30 || 0 == divider->hx || 0 == vnode->hx){
			obsv.fit_pos = FIT_LEFT;
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> FIT_LEFT", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			return obsv;
		}
	}
	//当前节点在当前分块的上面，无法适配
...
static bool is_suite_size(html_vnode_t *vnode, html_area_t *parentArea)
{
	if(vnode->wx*100 >= parentArea->area_info.width*50){
		if(vnode->hx >= SMALL_HX_VAL && vnode->hx*100 >= parentArea->area_info.height*10){
			return true;
		}
	}

	if(vnode->hx*100 >= parentArea->area_info.height*50){
		if(vnode->wx >= SMALL_HX_VAL && vnode->wx*100 >= parentArea->area_info.width*10){
			return true;
		}
	}
...
static bool is_main_table_vnode(html_vnode_t *vnode)
{
	...
	if(!vnode->firstChild || vnode->firstChild->nextNode || vnode->firstChild->hpNode->html_tag.tag_type != TAG_TABLE)
	{
		return false;
	}
	return true;
...
static bool is_main_ul_vnode(html_vnode_t *vnode)
{
	...
	if(!vnode->firstChild || vnode->firstChild->nextNode || vnode->firstChild->hpNode->html_tag.tag_type != TAG_UL)
	{
		return false;
	}
	return true;
 ...
 * @date 2011/07/05
**/
static bool is_need_divide(html_vnode_t *vnode, html_area_t *pArea,bool is_divid_text, dividing_t *divider)
{
	html_tag_type_t tag_type = vnode->hpNode->html_tag.tag_type;
	//叶子节点无法进行划分
	debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> ,subnodetype=%x,areawidth=%d,areaheight=%d, divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name,vnode->hpNode->subnodetype, pArea->area_info.width,pArea->area_info.height,__FILE__, __LINE__, __FUNCTION__);
	if (vnode->firstChild == NULL)
	{
		debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> don't have child, not divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
		return false;
	}
	if (vnode->struct_info->valid_child_num <= 2)
	{ //防止分块层次不好
		if (vnode->hpNode->subnodetype & (1 << 2))
		{ //子树上含有H（1－6）标签，继续划分（防止realtitle分块粒度过大）
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> ,subnodetype=%x,areawidth=%d,areaheight=%d, to be divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name,vnode->hpNode->subnodetype, pArea->area_info.width,pArea->area_info.height,__FILE__, __LINE__, __FUNCTION__);
			return true;
		}
	}
	if (IS_BORDER(vnode->property) && vnode->subtree_border_num == 1 && vnode->subtree_diff_font < 3)
	{ //具有border属性，且子树（包括自己）中只有自己具有border属性，且子树字体数目不超过2的不再继续划分 (sue)
		if (vnode->hx < 400 && vnode->wx < 800)
		{
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> not divide for it has border", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			return false;
		}
	}
	if(vnode->wx * 2 < vnode->vtree->root->wx && (is_h_ul_vnode(vnode) || is_main_ul_vnode(vnode)))
	{//防止被分得太细了
		debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> not divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
		return false;
	}
	if(vnode->subtree_textSize < 300 && is_main_table_vnode(vnode) && !is_contain_tag(vnode, TAG_TABLE) && divider->depth > 2 && vnode->hx < 400)
	{//防止table被分得太细了
	...
	if (vnode->struct_info->interaction_tag_num > 1 && (class_value != NULL || id_value != NULL))
	{
		bool flag = false;
		...
		if(flag)
		{
			...
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> not divide for it is interaction node", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			return false;
		}
	}
	if (divider->depth == 1 && vnode->hpNode->html_tag.tag_type == TAG_TABLE && vnode->upperNode && vnode->upperNode->hpNode->html_tag.tag_type == TAG_BODY)
	{
		if(vnode->hx < vnode->upperNode->hx - 20|| vnode->wx < vnode->upperNode->wx - 20)
		{
			//第一层分块时，遇到table标签认为其是布局用的，不要分碎了
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> is table of root, not divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			return false;
		}
	}
	if(divider->depth > 2)
	{
		...
		if (vnode->ypos >= 50 && vnode->ypos <= 360 && vnode->hx < 250 && vnode->hx >= 30 && vnode->wx >= 250)
		{
			if(vnode->hx < 50 && vnode->hpNode->html_tag.tag_type == TAG_TD && vnode->subtree_diff_font == 1)
			{
				debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> not divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
				return false;
			}
			bool flag = false;
			if(vnode->hpNode->html_tag.tag_type == TAG_P && vnode->subtree_diff_font <= 2)
			{
				flag = true;
			}
			if(!flag)
			{
				debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> divide for it maybe realtitle area", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
				return true;
			}
		}
		...
		if (vnode->hpNode->html_tag.tag_type == TAG_TABLE && divider->depth > 1 && vnode->subtree_textSize > 400)
		{
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			return true;
		}
	...
	if (divider->depth == 1)
	{
		...
		{
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> divide for it is too big", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			return true;
		}
	}
	//防止<h1>节点分细了
	...
	if (vnode->hpNode->html_tag.tag_type == TAG_H1 && child_num <= 3 && vnode->subtree_diff_font == 1)
	{
		debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> H1 not divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
		return false;
	}
	//文本节点无需划分
	if (!is_divid_text && IS_TEXT_VNODE(vnode->property))
	{
		debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> is text node, not divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
		return false;
	}
	//链接和会晃动的文字无需划分
	if (is_link(vnode) || vnode->hpNode->html_tag.tag_type == TAG_MARQUEE)
	{
		debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> is marrquee node, not divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
		return false;
	}
	//textarea节点无需划分
	if (vnode->hpNode->html_tag.tag_type == TAG_TEXTAREA)
	{
		debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> is textarea node, not divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
		return false;
	}
	//shuangwei add 20120605
	if ((vnode->hpNode->html_tag.tag_type == TAG_TABLE) && IS_DOMTREE_SUBTYPE(vnode->hpNode->owner->treeAttr) && !(vnode->hpNode->subnodetype & 31))
	{
		int colNum = 0;
		html_vnode_t *ptdNode = vnode;
		if (ptdNode && ptdNode->firstChild && ptdNode->firstChild->hpNode->html_tag.tag_type == TAG_THEAD)
		{
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> not divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			return false;
		}
		...
		if (ptdNode)
		{
			char *pcolspan = html_node_get_attribute_value(ptdNode->hpNode, "colspan");
			if (pcolspan && (((*pcolspan) > '1' && (*pcolspan) <= '9') || strlen(pcolspan) > 1))
			{
				debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> not divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
				return false;
			}
	...
	if ((vnode->hpNode->html_tag.tag_type == TAG_UL || vnode->hpNode->html_tag.tag_type == TAG_OL) && IS_DOMTREE_SUBTYPE(vnode->hpNode->owner->treeAttr) && !(vnode->hpNode->subnodetype & 15))
	{
		debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> not divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
		return false;
	}
	if (vnode->hpNode->html_tag.tag_type == TAG_DL && vnode->subtree_diff_font == 1)
	{
		debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> not divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
		return false;
	}
	if ((vnode->hpNode->html_tag.tag_type == TAG_FORM) && vnode->wx < 400 && vnode->hx < 400)
	{
		debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> not divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
		return false;
	}
	if (vnode->subtree_max_font_size >= 20 && vnode->subtree_diff_font == 2)
	{
		...
		{
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> divide for it maybe realtitle", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			return true;
		}
	}
	if (vnode->hpNode->html_tag.tag_type == TAG_P)
	{
		...
		if (IS_DOMTREE_SUBTYPE(vnode->hpNode->owner->treeAttr) && !(vnode->hpNode->subnodetype & 31) && (vnode->hpNode->owner->treeAttr & 5) != 0)
		{
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> not divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			return false;
		}
	...
	if (tag_type == TAG_TABLE)
	{
		...
		if (vnode->wx <= 600 && vnode->hx <= 200 && vnode->prevNode && vnode->nextNode && (vnode->ypos > 350 || vnode->ypos < 80))
		{
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> is table, not divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			return false;
		}
	}
	//节点的长宽和父分块一样大小，则该节点需要划分
	if (vnode->wx == pArea->area_info.width && vnode->hx == pArea->area_info.height && IS_DOMTREE_SUBTYPE(vnode->hpNode->owner->treeAttr) && (vnode->hpNode->subnodetype & 127))
	{
		bool flag = false; //是否豁免该策略

		html_tag_type_t tag_type = vnode->hpNode->html_tag.tag_type;
		if((tag_type == TAG_TABLE || tag_type == TAG_DIV) &&!(vnode->hpNode->subnodetype & 32)&& vnode->hx < 500 && vnode->wx < 500)
		{ //正文中会出现一些表格，不希望这样的表格被分细了。但对于布局用的表格，有的是需要继续划分的
			flag = true;
		}
		if(!flag)
		{
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> is equal area, divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			return true;
		}
	}
	...
	{
		if (vnode->struct_info && vnode->struct_info->is_self_repeat && vnode->struct_info->self_similar_value > 90)
		{
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> not divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			return false;
		}
	...
	if (vnode->wx * 100 > pArea->area_info.width * WX_PERCENT_BE_DIVIDE && vnode->hx * 100 > pArea->area_info.height * HX_PERCENT_BE_DIVIDE && vnode->wx >= 400 && vnode->hx >= 200 && can_vnode_split(vnode))
	{
		if (vnode->hpNode->html_tag.tag_type != TAG_TABLE)
		{
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
	...
	if (vnode->hx < 800)
	{
		if (vnode->subtree_diff_font > 1 && divider->depth > 1 && !vnode->struct_info->is_self_repeat && !vnode->struct_info->is_repeat_with_sibling)
		{
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> divide", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			return true;
		}
...
//	}
	...
	debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> not divide by default", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
	return false;
}
...
static bool is_divisible_by_config(html_vnode_t *vnode, const area_config_t *config)
{
	if(config && config->indivisible_tag_name
			&& vnode->hpNode->html_tag.tag_name
			&& strcasecmp(vnode->hpNode->html_tag.tag_name,config->indivisible_tag_name) == 0){
		return false;
	}
...
static observer_t check_area(html_vnode_t *vnode,dividing_t *divider)
{
	...
	//判断该节点是否可以被划分
	if(is_divisible_by_config(vnode,divider->config) && is_need_divide(vnode, pArea, is_divid_text, divider)){
		obsv.fit_pos = NO_FIT;
	...
	//如果是root节点(sue)
	html_tag_type_t tag_type = vnode->hpNode->html_tag.tag_type;
	if (tag_type == TAG_ROOT || tag_type == TAG_HTML || tag_type == TAG_BODY || tag_type == TAG_WAP_CARD)
	{
		obsv.fit_pos = NO_FIT;
		obsv.act = AREA_TOBE_DIVIDE;
		return obsv;
	...
	if (divider->last_valid)
	{
		...
		char *class_value = get_attribute_value(&vnode->hpNode->html_tag, ATTR_CLASS);
		char *last_class_value = get_attribute_value(&divider->last_valid->hpNode->html_tag, ATTR_CLASS);
		if (class_value && last_class_value && strcmp(class_value, last_class_value) != 0)
		{
			obsv.act = AREA_NEW_BEGIN;
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> act AREA_NEW_BEGIN", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			return obsv;
		}
	}
	if (vnode->hpNode->html_tag.tag_type == TAG_H1)
	{
		...
		debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> act AREA_END_BOTH", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
		return obsv;
	}
	...
	if(divider->depth > depth_limit && vnode->hpNode->html_tag.tag_type != TAG_P && vnode->struct_info->is_repeat_with_sibling && vnode->hx < 200)
	{
		if(!divider->last_valid || divider->last_valid->hpNode->html_tag.tag_type == vnode->hpNode->html_tag.tag_type)
		{
			if(divider->last_valid && vnode->hx>=20 && vnode->hx != divider->last_valid->hx)
			{
				obsv.act = AREA_NEW_BEGIN;
				debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> act AREA_NEW_BEGIN", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
				return obsv;
			}
			if(divider->last_valid && vnode->subtree_max_font_size != divider->last_valid->subtree_max_font_size)
			{
				obsv.act = AREA_NEW_BEGIN;
				debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> act AREA_NEW_BEGIN", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
				return obsv;
			...
			obsv.act = AREA_CONTINUE;
			debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> act AREA_CONTINUE for it is repeat with sibling", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			return obsv;
		}
	}
	...
	if(!is_divid_text && IS_TEXT_VNODE(vnode->property)){
		...
		if(divider->last_valid){
			if(vnode->hpNode->html_tag.tag_type == TAG_FONT)
			{
				...
				{
					if(divider->last_valid->subtree_max_font_size != vnode->subtree_max_font_size)
					{
						obsv.act = AREA_NEW_BEGIN;
						debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> act AREA_NEW_BEGIN", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
						return obsv;
	...
	if(divider->last_valid&&is_divid_text&&(IS_TEXT_VNODE(divider->last_valid->property)||divider->last_valid->hpNode->html_tag.tag_type==TAG_BR)&& IS_TEXT_VNODE(vnode->property)){
		obsv.act = AREA_CONTINUE;
		debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> act AREA_CONTINUE", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
		return obsv;
	}
	...
	switch(obsv.fit_pos){
		case NO_FIT:
			if(vnode->subtree_textSize == 0&&vnode->hpNode->html_tag.tag_type!=TAG_IMG&&vnode->hpNode->html_tag.tag_type!=TAG_DIV)
			{
				debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> act AREA_CONTINUE", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
				obsv.act = AREA_CONTINUE;
				return obsv;
			...
			if (vnode->nextNode && vnode->nextNode->hpNode->html_tag.tag_type == TAG_BR)
			{
				obsv.act = AREA_NEW_BEGIN;
				debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> act AREA_NEW_BEGIN", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			}else if(is_suite_size(vnode,pArea)&&(divider->last_valid||IS_BLOCK_TAG(vnode->property))&&vnode->hpNode->html_tag.tag_type != TAG_LI){
				obsv.act = AREA_END_BOTH;
				debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> act AREA_END_BOTH", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
			}
			else{
		...
		case FIT_LOWER:
			if(divider->last_valid)
			{
				...
				//子树文字长度为0的继续融合
				if(vnode->subtree_textSize == 0&&vnode->hpNode->html_tag.tag_type!=TAG_IMG)
				{
					debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> act AREA_CONTINUE", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
					obsv.act = AREA_CONTINUE;
					return obsv;
				...
				{
					if(vnode->hx < 200 && vnode->hpNode->html_tag.tag_type == divider->last_validword->hpNode->html_tag.tag_type)
					{
						if(vnode->firstChild && !vnode->firstChild->nextNode && divider->last_validword->firstChild && !divider->last_validword->firstChild->nextNode)
						{
							if(vnode->firstChild->hpNode->html_tag.tag_type == divider->last_validword->firstChild->hpNode->html_tag.tag_type)
							{
								debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> act AREA_CONTINUE", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
								obsv.act = AREA_CONTINUE;
								return obsv;
			...
			if(vnode->hx >= SMALL_HX_VAL&&vnode->hpNode->html_tag.tag_type != TAG_BR &&vnode->hpNode->html_tag.tag_type != TAG_PURETEXT){
				obsv.act = AREA_END_BOTH;
				debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> act AREA_END_BOTH", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
				return obsv;
			}
		...
		case FIT_RIGHT:
			if(vnode->hpNode->html_tag.tag_type!=TAG_LI)
			{
				...
				if(vnode->wx >= SMALL_WX_VAL&&(vnode->hpNode->html_tag.tag_type==TAG_BR)){
						obsv.act = AREA_END;
						debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> act AREA_END", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
						return obsv;
				}
			}
			obsv.act = AREA_CONTINUE;
 ...
 * @date 2011/07/05
**/
static void block_append(dividing_t *divider,html_vnode_t *vnode,unsigned int fit_pos)
{
	//该分块目前为空，则填充开始和结束节点
	if(divider->begin == NULL){
		divider->begin = vnode;
 ...
 * @date 2011/07/05
**/
static bool is_divide_nothing(html_vnode_t *tail_vnode, html_area_t *parentArea)
{
...
{
	if(divider->begin != NULL && !is_little_block(divider) && !is_divide_nothing(divider->end, divider->parent_area)){
...
static int end_visit_for_divide(html_vnode_t *vnode, void *result)
{
	...
	debuginfo(DIVIDING_AREA, "[depth:%d] node(id=%d)<%s> old_collect_pass_away", divider->depth, vnode->id, vnode->hpNode->html_tag.tag_name);
	return VISIT_NORMAL;
}
...
//	for(;!validArea->isValid;validArea=validArea->nextArea);
	...
	if(validArea->subArea == NULL){
		...
		for( html_area_t *subArea = validArea->subArea; subArea; subArea=subArea->nextArea){
 ...
 * @date 2011/07/05
**/
static inline bool is_in_limit(const  html_area_t *area, unsigned int depth, const area_config_t *cfg)
{
...
int areaNode_divide(html_area_t *area, const area_config_t *cfg, nodepool_t *np, unsigned int depth)
{
	...
	{
		debuginfo(DIVIDING_AREA, "[depth:%d] area(node id=%d) not divide", area->depth, area->begin->id);
		return 1;
	}
	...
	{
		if (for_partition_area && for_partition_area->struct_info->valid_leaf_num == 1)
		{
			debuginfo(DIVIDING_AREA, "[depth:%d] area(node id=%d) not divide", area->depth, area->begin->id);
			return 1;
		}
	...
	if (area->begin == area->end)
	{
		...
		int hx = vnode->hx;
		int wx = vnode->wx;
		if (class_value && (strstr(class_value, "copyright") || strstr(class_value, "footer") || strstr(class_value, "discuss")))
		{ //可能是版权的分块不再细分
			debuginfo(DIVIDING_AREA, "[depth:%d] area(node id=%d) not divide", area->depth, area->begin->id);
			return 1;
		}
		if (depth > 2)
		{
			if (hx <= 50 && (class_value != NULL || id_value != NULL))
			{ //高度在一定范围内，且具有class或id属性的不再继续划分
				debuginfo(DIVIDING_AREA, "[depth:%d] area(node id=%d) not divide", area->depth, area->begin->id);
				return 1;
			}
		}
		if (depth > 2)
		{
			if (vnode->struct_info->is_self_repeat && vnode->struct_info->self_similar_value >= 96)
			{ //重复父结构不再继续划分
				debuginfo(DIVIDING_AREA, "[depth:%d] area(node id=%d) not divide for it is self repeat area", area->depth, area->begin->id);
				return 1;
			}
		}
		...
		if (!(vnode->hpNode->subnodetype && (1<<5)))
		{//不包含form节点
			if (vnode->hpNode->html_tag.tag_type == TAG_TABLE && wx >= 700 && hx <= 40 && subtree_node_num < 100)
			{ //长条形状的table节点，且不包含form节点的，不再细分 (sue)
				debuginfo(DIVIDING_AREA, "[depth:%d] area(node id=%d) not divide for it is like navigation", area->depth, area->begin->id);
				return 1;
			}
		}
	}
	...
	for (html_vnode_t *vnode = area->begin; vnode; vnode = vnode->nextNode)
	{
		int ret = html_vnode_visit(vnode, visit_for_divide, end_visit_for_divide, &divider);
		if (ret == VISIT_ERROR)
		{
			goto FAIL;
		}
		if (vnode == area->end)
		{
			if (area->valid_subArea_num > 0 && old_collect_pass_away(&divider) == -1)
			{
				goto FAIL;
			}
			break;
	...
	//本层有效分块少于1个，将丢弃
	if (is_cut_vlevel || (area->valid_subArea_num == 0 && area->subArea_num > 0))
	{
		new_depth--;
	}
	// 继续划分子分块
	...
	else if (area->valid_subArea_num == 0 && area->subArea_num > 0)
	{ //没有有效分块的层
...
static int visit_for_add_no(html_area_t *area, void *data)
{
	...
	area->no = (*no)++;
	if (area->isValid && area->depth >= (unsigned int) area->area_tree->max_depth)
	{
 ...
 * @date 2011/07/05
**/
int areatree_visit(html_area_t * area, FUNC_START_T start, FUNC_FINISH_T finish, void * data)
{
...
void printArea(html_area_t * area,int level){
	...
	if(area){
		myprintf("***********************************\n");
		myprintf("the begin node of the area with depth=%d :\n",level);
		printNode(area->begin->hpNode);
		myprintf("the area postion is (id=%d,x=%d,y=%d,height=%d,width=%d,isvalid=%d)\n",area->no,area->area_info.xpos,area->area_info.ypos,area->area_info.height,area->area_info.width,area->isValid);
        myprintf("Its srctype=%02X,functype=%02X,semtype=%02X\n",area->srctype_mark._mark_bits,area->func_mark._mark_bits,area->sem_mark._mark_bits);
		for(html_area_t * subarea=area->subArea;subarea;subarea=subarea->nextArea){
			printArea( subarea,level+1);
		}
