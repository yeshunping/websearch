...
bool process_one_page(char* url, int urlLen, char* page, int pageLen, classify_res_t* res)
{
	...
	int ret = html_tree_extract_stdata(url, urlLen, 0, res->tree, res->stdata);
	if (ret != 0)
	{
	...
	printf("\ntype=%d\turl=%s\n", res->stdata->type, url);
        for(vector<StructureKV>::iterator j=res->stdata->all.begin(); j!=res->stdata->all.end(); j++)
