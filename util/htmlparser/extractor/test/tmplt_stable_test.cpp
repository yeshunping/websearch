...
void* consume_thread(void* p)
{
	...
	tmplt_result_t* result = create_tmplt_result(); //单线程使用tmplt_extract_data_t
	assert(result);
...
int main(int argc, char** argv)
{
	...
	templates_t* dict = create_templates("../conf"); //多线程使用templates_t
//	templates_t* dict = create_templates("extractor/conf"); //多线程使用templates_t
	assert(dict);
	int try_count = 0;
	int load_ret = 0;
