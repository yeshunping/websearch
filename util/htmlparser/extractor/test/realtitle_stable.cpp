...
bool process_one_page(char* url, int urlLen, char* page, int pageLen, classify_res_t* res)
{
	...
	timestart();
	char realtitle[2048];
	int len = html_atree_extract_realtitle(res->atree, realtitle, 2048);
	timeend("html_atree_extract_realtitle", ""); //打印时间调试信息
	printf("[html_atree_extract_realtitle] - len:%d,%s,%s\n", len, realtitle, url);

	//从dom树上抽取tagtitle
	timestart();
	char tagtitle[2048];
	int tagtitle_len = html_tree_extract_tagtitle(res->tree, tagtitle, 2048);
	timeend("html_tree_extract_tagtitle", "");
	printf("[html_tree_extract_tagtitle] - len:%d,%s,%s\n", tagtitle_len, tagtitle, url);

	//带有优化策略的标题抽取
	timestart();
	char realtitle2[2048];
	realtitle_input_t rt_input;
	rt_input.atree = res->atree;
	rt_input.url = url;
	rt_input.url_len = urlLen;
	...
	printf("[easou_extract_realtitle] - len:%d,%s,%s\n", rt_len2, realtitle2, url);
...
void* consume_thread(void* p)
{
	...
	while (true)
	{
		...
		{
			pthread_mutex_unlock(&mutex);
			printf("url:%d css_request:%d css_missing:%d\n", res->vtree_in->url_num, res->vtree_in->request_css_num, res->vtree_in->missing_css_num);
			return NULL;
		}
		pthread_mutex_unlock(&mutex);
