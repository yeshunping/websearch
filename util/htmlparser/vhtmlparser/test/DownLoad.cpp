...
int DownLoad::download(char* url, int url_len, char* buf, int buf_len, char*& result)
{
	if(url_len > 1020)
		return -1;
...
int DownLoad::down(char* url, int url_len, char* buf, int buf_len, char*& result)
{
	char host_addr[1024];
	char host_file[1024];
	int port;
	...
	while(begin < end)
	{
		...
		if(strnicmp(begin, "meta ", 5) == 0)
		{
			...
			if(req && pt && url)
			{
				...
				{
					char* p1 = jump_url;
					while(url < begin && *url != '\"' && *url != ' ') *p1++ = *url++;
					*p1 = 0;
					return p1 - jump_url;
				}
			}
...
int DownLoad::GetHost(char* url, int url_len, char* addr, char* file, int* port)
{
	char* p1 = url, *p2 = url + url_len, *p3 = addr, *p4 = file;

	while(p1 < p2 && *(short*)p1 != *(short*)"//") p1++;
	p1 += 2;
	if(p1 >= p2)
		return -1;
...
int DownLoad::GetRequestHead(char* host_addr, char* host_file, int port, char* head_buf)
{
...
//	p += sprintf(p, "User-Agent: Opera/9.51 (Windows NT 5.1; U; zh-cn)\r\n");
	...
	p += sprintf(p, "Host: %s\r\n", host_addr);
	p += sprintf(p, "Accept:text/html, application/xml;q=0.9, application/xhtml+xml, image/x-xbitmap, */*;q=0.1\r\n");
	p += sprintf(p, "If-Modified-Since:%sGMT\r\n\r\n", lastmodify);
	
	return p - head_buf;
}

int DownLoad::ParseHead(char* buf, int buf_len, char*& page, int& page_len, char* jump_url, int& jump_len)
{
	buf[buf_len] = 0;
	...
	{
		char* location = begin;
		while(begin < end && *begin != 0x0a) begin++;
		while(begin < end && (*begin == '\r' || *begin == '\n')) begin++; 
		if(strnicmp(location, "Location", 8) == 0)
		{
			location += 8;
			while(location < begin && (*location == ':' || *location == ' ')) location++;
			jump_len = 0;
			if(begin - location >= 1024)
				return -1;
			while(location < begin) jump_url[jump_len++] = *location++;
			jump_url[jump_len--] = 0;
			while(jump_len >= 0 && (jump_url[jump_len] == '\r' || jump_url[jump_len] == '\n' || jump_url[jump_len] == ' '))
				jump_len--;
			jump_url[++jump_len] = 0;
			if(station == 301 || station == 302)
				return station;
...
int DownLoad::to_real_addr(char* base, char* tmp_url, char* url, char* tmp_buf)
{
