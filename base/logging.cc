...
DEFINE_string(logging_dir, "log", "dir to save log messages");
DEFINE_int64(max_log_file_size, 1024L * 1024 * 1024 * 10, "max log file size");
...
PathString* backup_log_file_name = NULL;
...
bool InitializeLogFileHandle() {
  if (log_file) {
    if (ftell(log_file) < FLAGS_max_log_file_size) {
      return true;
    }
    printf("log file larger than expected size, backup\n");
    fflush(stdout);
    fclose(log_file);
    log_file = NULL;
    //  move log file to backup file,
    string history_log_file = StringPrintf("%s.%ld",
                                           log_file_name->c_str(),
                                           time(NULL));
    rename(log_file_name->c_str() , history_log_file.c_str());
  }
...
void InitLogging(const PathChar* prefix, LoggingDestination logging_dest,
  ...
  //  If log dir not exist, create it.
  if(access(FLAGS_logging_dir.c_str(), 0) != 0) {
     mkdir(FLAGS_logging_dir.c_str(), 0755);
  }
  ...
  if (!backup_log_file_name)
    backup_log_file_name = new PathString();
  *log_file_name = StringPrintf("%s/%s.log", FLAGS_logging_dir.c_str(), prefix);
  *backup_log_file_name =
      StringPrintf("%s/%s.log.bak", FLAGS_logging_dir.c_str(), prefix);
