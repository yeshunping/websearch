...
#include "base/thirdparty/dmg_fp/dmg_fp.h"
#include "base/utf_string_conversion_utils.h"
#include "base/thirdparty/icu/icu_utf.h"
...
uint32 StringToUint(const std::string& value) {
  if (value.empty())
    return 0;
...
uint64 StringToUint64(const std::string& value) {
  if (value.empty())
    return 0;
  return strtoull(value.c_str(), NULL, 10);
}
bool StringToUint64(const std::string& value, uint64* out) {
  if (value.empty())
    return false;
...
void base::StringLowerCopy(char* dst, const char* src, size_t size) {
...
std::string JoinVector(std::vector<std::string> values, char ch) {
